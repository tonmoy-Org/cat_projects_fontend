...
// ── Status helpers (shared with OrdersClient) ──────────────────────────────
const useStatusStyles = () => {
  ...
  const getOrderStatusStyle = (status) => {
    switch ((status || '').toLowerCase()) {
      case 'delivered':
      case 'completed':
        return { bg: alpha(GREEN, dark ? 0.2 : 0.1), color: GREEN };
      case 'shipped':
        return { bg: alpha(WARNING, dark ? 0.2 : 0.1), color: WARNING };
      case 'pending':
        return { bg: alpha(INFO, dark ? 0.2 : 0.1), color: INFO };
      case 'cancelled':
        return { bg: alpha(ERROR, dark ? 0.2 : 0.1), color: ERROR };
      default:
        return { bg: alpha('#888', 0.1), color: '#888' };
    }
  };
  ...
  return { getOrderStatusStyle, getPaymentStatusStyle };
};
...
export const ClientDashboard = () => {
  ...
  const { getOrderStatusStyle, getPaymentStatusStyle } = useStatusStyles();
  ...
  // ── Recent 3 orders (excluding the top one) ───────────────────────────
  ...
  // ── Stat card data ────────────────────────────────────────────────────
  const statCards = [
    { label: 'Total Orders', value: stats.totalOrders, icon: <ShoppingCartIcon sx={{ fontSize: '1.1rem' }} />, color: BLUE },
    { label: 'Total Spent', value: `৳${parseInt(stats.totalSpent).toLocaleString()}`, icon: <PaymentIcon sx={{ fontSize: '1.1rem' }} />, color: GREEN },
    { label: 'Delivered', value: stats.completed, icon: <CheckCircleIcon sx={{ fontSize: '1.1rem' }} />, color: GREEN },
    { label: 'In Transit', value: stats.shipped, icon: <ShippingIcon sx={{ fontSize: '1.1rem' }} />, color: WARNING },
    { label: 'Pending', value: stats.pending, icon: <ClockIcon sx={{ fontSize: '1.1rem' }} />, color: INFO },
    { label: 'Paid', value: stats.paid, icon: <TrendingUpIcon sx={{ fontSize: '1.1rem' }} />, color: GREEN },
  ];
  ...
  return (
    <Box>
      ...
      >
        ...
        <Box>
          <Typography sx={{ fontWeight: 700, color: TEXT, fontSize: '1rem', lineHeight: 1.3 }}>
            Welcome back{user?.name ? `, ${user.name}` : ''}! 👋
          </Typography>
          <Typography sx={{ fontSize: '0.78rem', color: alpha(TEXT, 0.55), mt: 0.25 }}>
          ...
          <Grid size={{ xs: 6, sm: 4, md: 2 }} key={label}>
            <Card elevation={0} sx={cardSx(color)}>
              <CardContent sx={{ p: '14px !important', display: 'flex', flexDirection: 'column', gap: 0.75 }}>
                ...
                <Typography sx={{ fontSize: '1.2rem', fontWeight: 700, color: TEXT, lineHeight: 1 }}>
                  {value}
                </Typography>
      ...
      <Box mb={3} p={2} sx={{ borderRadius: 2, border: `1px solid ${DIVIDER}`, bgcolor: BG }}>
        <Box display="flex" justifyContent="space-between" alignItems="center" mb={1}>
          <Typography sx={{ fontSize: '0.82rem', fontWeight: 600, color: TEXT }}>Order Completion Rate</Typography>
          <Typography sx={{ fontSize: '0.82rem', fontWeight: 700, color: GREEN }}>{stats.completionRate}%</Typography>
        </Box>
        <LinearProgress
          variant="determinate"
          ...
          sx={{
            ...
            '& .MuiLinearProgress-bar': { borderRadius: 4, bgcolor: GREEN },
          }}
        />
        <Box display="flex" justifyContent="space-between" mt={0.75}>
          <Typography sx={{ fontSize: '0.7rem', color: alpha(TEXT, 0.45) }}>{stats.completed} delivered</Typography>
          <Typography sx={{ fontSize: '0.7rem', color: alpha(TEXT, 0.45) }}>{stats.totalOrders} total</Typography>
        </Box>
      </Box>
          ...
          <Card elevation={0} sx={{ ...cardSx(BLUE), height: 'auto' }}>
            <CardContent sx={{ p: '20px !important' }}>
              <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
                <Typography sx={{ fontWeight: 700, fontSize: '0.9rem', color: TEXT }}>
                  Most Recent Order
                </Typography>
                <Box
                  ...
                  sx={{
                    ...
                    color: BLUE, cursor: 'pointer', fontSize: '0.75rem', fontWeight: 600,
                    '&:hover': { opacity: 0.75 },
                  }}
                >
                  View All <ArrowForwardIcon sx={{ fontSize: '0.85rem' }} />
                </Box>
              </Box>
              ...
              {isLoading ? (
                ...
                <Box>
                  ...
                  >
                    <Box display="flex" justifyContent="space-between" alignItems="center">
                      <Typography sx={{ fontWeight: 700, color: BLUE, fontSize: '0.9rem' }}>
                    ...
                    {[
                      ...
                      <Grid size={6} key={label}>
                        <Box p={1.25} sx={{ borderRadius: 1.5, border: `1px solid ${DIVIDER}` }}>
                          <Typography sx={labelSx}>{label}</Typography>
                          <Typography sx={{ fontWeight: 700, fontSize: '0.95rem', color, mt: 0.25 }}>
                            {value}
                          </Typography>
                        </Box>
                      </Grid>
                    ...
                    {(() => {
                      ...
                      return (
                        <Chip
                          ...
                          size="small"
                          sx={{
                            fontWeight: 600, fontSize: '0.73rem', height: 24,
                    ...
                    <Box>
                      ...
                      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.75 }}>
                          ...
                          >
                            <Box display="flex" alignItems="center" gap={1}>
                              <Box sx={{
                                ...
                                <BoxIcon sx={{ fontSize: '0.8rem', color: BLUE }} />
                              </Box>
                              <Typography sx={{ fontSize: '0.82rem', color: TEXT, fontWeight: 500 }}>
                                {item.name}
                              </Typography>
                            </Box>
                            <Chip
                        ...
                        {recentOrder.items.length > 4 && (
                          <Typography sx={{ fontSize: '0.75rem', color: alpha(TEXT, 0.45), textAlign: 'center', pt: 0.5 }}>
                ...
                <Box py={4} textAlign="center">
                  ...
                  <Typography sx={{ fontSize: '0.82rem', color: alpha(TEXT, 0.4) }}>No orders yet</Typography>
                </Box>
              )}
            </CardContent>
          </Card>
          ...
          <Card elevation={0} sx={{ ...cardSx(BLUE), height: 'auto' }}>
            <CardContent sx={{ p: '20px !important' }}>
              <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
                ...
                >
                  See All <ArrowForwardIcon sx={{ fontSize: '0.85rem' }} />
                </Box>
              </Box>
              ...
              {isLoading ? (
                ...
                <Box py={4} textAlign="center">
                  <ShoppingCartIcon sx={{ fontSize: 40, color: alpha(TEXT, 0.15), mb: 1 }} />
                  <Typography sx={{ fontSize: '0.82rem', color: alpha(TEXT, 0.4) }}>No previous orders</Typography>
                </Box>
              ) : (
                        ...
                        >
                          <Box display="flex" alignItems="center" gap={1.5}>
                            <Box sx={{
                              ...
                              <ShoppingCartIcon sx={{ fontSize: '0.95rem', color: s.color }} />
                            </Box>
                            <Box>
                              <Typography sx={{ fontWeight: 700, fontSize: '0.83rem', color: TEXT }}>
                                #{order.orderId || order._id?.slice(-8)}
                              </Typography>
                              <Typography sx={{ fontSize: '0.72rem', color: alpha(TEXT, 0.45) }}>
                                {order.createdAt
                          ...
                          <Box textAlign="right">
                            <Typography sx={{ fontWeight: 700, fontSize: '0.85rem', color: GREEN }}>
