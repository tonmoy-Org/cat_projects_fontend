...
const defaultFormValues = {
  name: "", age: "", breed: "", about: "",
  gender: "", neutered: false, vaccinated: false, size: "medium",
};
...
export default function CatsManagement() {
  ...
  // Gallery state: { file, preview, isExisting }
  // isExisting=true means it came from the server (URL string), not a new upload
  ...
  const handleOpenModal = (mode, cat = null) => {
    ...
    if (cat) {
      ...
      reset({
        ...
        about: cat.about || "", gender: cat.gender || "",
  ...
  const buildFormData = (data) => {
    ...
    // Featured image (new file or keep existing)
    if (featuredImageFile) {
      formData.append("featuredImage", featuredImageFile);
    }

    // Existing gallery URLs (so backend knows what to keep)
    ...
    // New gallery files
    galleryItems.filter(i => !i.isExisting).forEach(item => {
  ...
  const deleteCatMutation = useMutation({
    mutationFn: async (id) => { const response = await axiosInstance.delete(`/cats/${id}`); return response.data; },
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["cats"] }); addAlert("success", "Cat profile deleted successfully!"); setDeleteDialogOpen(false); setCatToDelete(null); },
    onError: (error) => { addAlert("error", error.response?.data?.message || "Failed to delete cat profile."); setDeleteDialogOpen(false); setCatToDelete(null); },
  });
  ...
  const isSubmitDisabled = () => {
    ...
    if (modalMode === "create") return !isValid || !featuredPreview;
  ...
  return (
    <Box>
      ...
      <Dialog open={deleteDialogOpen} onClose={handleDeleteCancel} maxWidth="xs" fullWidth>
        <DialogTitle sx={{ color: TEXT_PRIMARY, fontWeight: 600 }}>Confirm Delete</DialogTitle>
        <DialogContent>
      ...
      <TableContainer component={Paper} elevation={0} sx={{ borderRadius: 1, border: `1px solid ${theme.palette.divider}`, backgroundColor: theme.palette.background.paper, overflow: "hidden" }}>
          ...
          <TableBody>
            {isLoading ? (
              ...
              <TableRow key={cat._id} hover sx={{ "&:hover": { backgroundColor: alpha(BLUE_COLOR, 0.03) }, "&:last-child td": { borderBottom: 0 } }}>
                ...
                <TableCell sx={{ py: 1 }}><Typography variant="body2" sx={{ fontWeight: 500, color: TEXT_PRIMARY }}>{cat.name}</Typography></TableCell>
                <TableCell sx={{ py: 1 }}>
                ...
                <TableCell sx={{ py: 1 }}><Typography variant="body2">{cat.age}</Typography></TableCell>
                <TableCell sx={{ py: 1 }}><Typography variant="body2">{cat.breed}</Typography></TableCell>
                <TableCell sx={{ py: 1 }}>
                  <Box sx={{ display: "flex", gap: 0.5, flexWrap: "wrap" }}>
                    {cat.neutered && <Chip label="Neutered" size="small" sx={{ fontSize: "0.7rem", height: 20, backgroundColor: alpha(GREEN_COLOR, 0.1), color: GREEN_COLOR }} />}
                    {cat.vaccinated && <Chip label="Vaccinated" size="small" sx={{ fontSize: "0.7rem", height: 20, backgroundColor: alpha(BLUE_COLOR, 0.1), color: BLUE_COLOR }} />}
                  </Box>
                </TableCell>
                <TableCell sx={{ py: 1 }}>
                  <Box sx={{ display: "flex", alignItems: "center", gap: 0.5 }}>
        ...
        <TablePagination rowsPerPageOptions={[5, 10, 25, 50]} component="div" count={cats.length} rowsPerPage={rowsPerPage} page={page} onPageChange={handleChangePage} onRowsPerPageChange={handleChangeRowsPerPage} sx={{ borderTop: `1px solid ${theme.palette.divider}` }} />
      </TableContainer>

      {/* Modal */}
      <Dialog open={openModal} onClose={handleCloseModal} maxWidth="lg" fullWidth PaperProps={{ sx: { borderRadius: 1, backgroundColor: theme.palette.background.paper, maxHeight: "90vh" } }}>
              ...
              <Grid size={{ xs: 12, md: 5 }}>
                ...
                <Box sx={{ mb: 3 }}>
                  ...
                  {modalMode !== "view" ? (
                    ...
                    featuredPreview && (
                      <PreviewWrapper>
                        <img src={featuredPreview} alt={formValues.name} style={{ width: "100%", maxHeight: 250, objectFit: "cover", borderRadius: 4 }} />
                      </PreviewWrapper>
                    )
                  )}
                </Box>
                  ...
                  <Grid container spacing={2}>
                    <Grid size={{ xs: 12 }}>
                      ...
                      <Controller name="name" control={control}
                        ...
                        render={({ field }) => (
                          <StyledTextField {...field} fullWidth size="small" disabled={modalMode === "view"} error={!!errors.name} helperText={errors.name?.message} placeholder="Enter cat's name"
                            onChange={(e) => { field.onChange(e); setEditFormDirty(true); }} />
                        )} />
                    </Grid>
                      ...
                      <Controller name="gender" control={control} rules={{ required: "Gender is required" }}
                        render={({ field }) => (
                          <FormControl fullWidth size="small" error={!!errors.gender} disabled={modalMode === "view"}>
                            <Select {...field} displayEmpty onChange={(e) => { field.onChange(e); setEditFormDirty(true); }}>
                    ...
                    <Grid size={{ xs: 6 }}>
                      ...
                      <Controller name="neutered" control={control}
                        render={({ field }) => (
                          <FormControlLabel control={<Switch {...field} checked={field.value} onChange={(e) => { field.onChange(e.target.checked); setEditFormDirty(true); }} disabled={modalMode === "view"} color="primary" size="small" />}
                      ...
                      <Controller name="size" control={control}
                        render={({ field }) => (
                          <FormControl fullWidth size="small" disabled={modalMode === "view"}>
                            <Select {...field} onChange={(e) => { field.onChange(e); setEditFormDirty(true); }}>
