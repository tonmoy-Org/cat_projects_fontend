...
export const UserManagement = () => {
    ...
    const createUserMutation = useMutation({
        ...
        onError: (err) => {
            addAlert('error', err.response?.data?.message || 'Failed to create user');
        },
    });
    ...
    const updateUserMutation = useMutation({
        ...
        onError: (err) => {
            addAlert('error', err.response?.data?.message || 'Failed to update user');
        },
    });
    ...
    const toggleUserStatusMutation = useMutation({
        ...
        onError: (err) => {
            addAlert('error', err.response?.data?.message || 'Failed to update user status');
            setOpenStatusDialog(false);
            setUserToToggle(null);
        },
    });
    ...
    const handleToggleStatusConfirm = () => {
        if (userToToggle) toggleUserStatusMutation.mutate(userToToggle._id);
    };
    ...
    // Support both id and _id from auth context
    ...
    return (
        <Box>
                                    ...
                                    >
                                                    ...
                                                    <Typography variant="body2" fontWeight={500} sx={{ fontSize: '0.85rem', color: TEXT_PRIMARY }}>
                                                        ...
                                                        {isOwnAccount && (
                                                            <Chip
                                                                label="You"
                                                                size="small"
                                                                sx={{ ml: 1, backgroundColor: alpha(BLUE_COLOR, 0.1), color: BLUE_COLOR, fontSize: '0.65rem', height: 18 }}
                                                            />
                                                        )}
                                                    </Typography>
                                        ...
                                        <TableCell sx={{ py: 1.5 }}>
                                            <Chip
                                                ...
                                                size="small"
                                                sx={{ fontWeight: 500, fontSize: '0.75rem', height: 24, ...getRoleStyle(user.role) }}
                                            />
                                        </TableCell>
                                        <TableCell sx={{ py: 1.5 }}>
                                            <Chip
                                                ...
                                                sx={{ fontWeight: 500, fontSize: '0.75rem', height: 24, ...getStatusStyle(user.isActive) }}
                                            />
                                        </TableCell>
                                            ...
                                            <Tooltip title={isOwnAccount ? "Cannot change your own status" : (user.isActive ? "Deactivate User" : "Activate User")}>
                                                <span>
                                                    <IconButton
                                                        ...
                                                        disabled={isOwnAccount}
                                                        sx={{ color: user.isActive ? RED_COLOR : GREEN_COLOR, fontSize: '0.9rem', mr: 0.5 }}
                                                    >
                                                        {user.isActive ? <BlockIcon fontSize="inherit" /> : <CheckCircleIcon fontSize="inherit" />}
                                                    </IconButton>
                                                </span>
                                            </Tooltip>
            ...
            <Dialog open={openDialog} onClose={handleCloseDialog} maxWidth="sm" fullWidth PaperProps={{ sx: { borderRadius: 2 } }}>
                <DialogTitle sx={{ color: TEXT_PRIMARY, fontWeight: 600, fontSize: '1rem', py: 2, px: 3, borderBottom: `1px solid ${theme.palette.divider}` }}>
                ...
                <DialogContent sx={{ px: 3, py: 2 }}>
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 1 }}>
                        ...
                        <StyledTextField
                            ...
                            size="small" placeholder={selectedUser ? 'Enter new password' : 'Enter password'}
                        />
                        ...
                        {selectedUser && (
                            <FormControlLabel
                                control={<Switch checked={formData.isActive} onChange={handleSwitchChange} name="isActive" color="primary" size="small" />}
                                label={<Typography variant="body2" sx={{ fontSize: '0.85rem', color: TEXT_PRIMARY }}>Active</Typography>}
                            />
                        )}
                    </Box>
                </DialogContent>
                <DialogActions sx={{ px: 3, py: 2, borderTop: `1px solid ${theme.palette.divider}` }}>
                    <OutlineButton onClick={handleCloseDialog} size="medium" sx={{ fontSize: '0.85rem', px: 2 }} disabled={createUserMutation.isPending || updateUserMutation.isPending}>
                        Cancel
                    </OutlineButton>
                    <GradientButton
            ...
            <Dialog open={openDeleteDialog} onClose={() => setOpenDeleteDialog(false)} maxWidth="xs" fullWidth PaperProps={{ sx: { borderRadius: 2 } }}>
                <DialogTitle sx={{ color: RED_COLOR, fontWeight: 600, fontSize: '1rem', py: 2, px: 3, borderBottom: `1px solid ${theme.palette.divider}` }}>
                ...
                <DialogActions sx={{ px: 3, py: 2, borderTop: `1px solid ${theme.palette.divider}` }}>
                    <OutlineButton onClick={() => setOpenDeleteDialog(false)} size="medium" sx={{ fontSize: '0.85rem', px: 2 }} disabled={deleteUserMutation.isPending}>
                        Cancel
                    </OutlineButton>
                    <Button
                        variant="contained"
                        sx={{ background: `linear-gradient(135deg, ${RED_DARK} 0%, ${RED_COLOR} 100%)`, color: 'white', borderRadius: 1, padding: '6px 16px', fontWeight: 500, fontSize: '0.85rem', textTransform: 'none', '&:hover': { background: `linear-gradient(135deg, ${RED_COLOR} 0%, #b91c1c 100%)` } }}
                        onClick={handleDeleteConfirm}
                        startIcon={deleteUserMutation.isPending ? <CircularProgress size={18} sx={{ color: 'white' }} /> : <DeleteIcon sx={{ fontSize: '0.9rem' }} />}
                        size="medium" disabled={deleteUserMutation.isPending}
                    >
            ...
            <Dialog open={openStatusDialog} onClose={() => setOpenStatusDialog(false)} maxWidth="xs" fullWidth PaperProps={{ sx: { borderRadius: 2 } }}>
                <DialogTitle sx={{ color: userToToggle?.isActive ? RED_COLOR : GREEN_COLOR, fontWeight: 600, fontSize: '1rem', py: 2, px: 3, borderBottom: `1px solid ${theme.palette.divider}` }}>
                    <Box display="flex" alignItems="center" gap={1}>
                        {userToToggle?.isActive ? <BlockIcon sx={{ fontSize: '1.1rem' }} /> : <CheckCircleIcon sx={{ fontSize: '1.1rem' }} />}
                        Confirm Status Change
                    </Box>
                </DialogTitle>
                <DialogContent sx={{ px: 3, py: 2 }}>
                    <DialogContentText sx={{ fontSize: '0.9rem', color: TEXT_PRIMARY }}>
                        Are you sure you want to {userToToggle?.isActive ? 'deactivate' : 'activate'} the user <strong>"{userToToggle?.name}"</strong>?
                    </DialogContentText>
                </DialogContent>
                <DialogActions sx={{ px: 3, py: 2, borderTop: `1px solid ${theme.palette.divider}` }}>
                    <OutlineButton onClick={() => setOpenStatusDialog(false)} size="medium" sx={{ fontSize: '0.85rem', px: 2 }} disabled={toggleUserStatusMutation.isPending}>
                        Cancel
                    </OutlineButton>
                    <GradientButton
                        variant="contained" onClick={handleToggleStatusConfirm} disabled={toggleUserStatusMutation.isPending}
                        startIcon={toggleUserStatusMutation.isPending ? <CircularProgress size={18} sx={{ color: 'white' }} /> : (userToToggle?.isActive ? <BlockIcon sx={{ fontSize: '0.9rem' }} /> : <CheckCircleIcon sx={{ fontSize: '0.9rem' }} />)}
                        size="medium" sx={{ fontSize: '0.85rem', px: 2 }}
                    >
                        {toggleUserStatusMutation.isPending ? 'Updating...' : userToToggle?.isActive ? 'Deactivate' : 'Activate'}
                    </GradientButton>
                </DialogActions>
            </Dialog>
        </Box>
